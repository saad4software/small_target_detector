...
while True:
    ...
    for index, box in enumerate(bboxes):
        ...
        if is_target:
            cv2.imwrite(f"yolo_ds/images/{file_name}_{counter}.png", frame2)
            f = open(f'yolo_ds/labels/{file_name}_{counter}.txt', "w")
            f.write(f'{0} {x_min+rw/2} {y_min+rh/2} {rw} {rh}\r\n')
            f.close()
    ...
    # cv2.imwrite(f'frame{counter}.png', frame2)
